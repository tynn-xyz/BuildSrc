...
class PublishingAndroidSourcesTest {
    ...
    @DisplayName('build should create all Android sources')
    void shouldBuildAndroidSources(g, a, hasKotlin) {
        buildFile << """
        ...
        println new File(projectDir, 'build/outputs/sources').listFiles()
    ...
    @ParameterizedTest(name = "Gradle={0} Android={1} Kotlin={2}")
    @MethodSource("gradleAndAndroidVersions")
    @DisplayName('publishToMavenLocal should publish all Android sources')
    void shouldPublishAndroidSources(g, a, hasKotlin) {
        buildFile << """
            apply plugin: 'com.android.library'
    ...
    @ParameterizedTest(name = "Gradle={0} Android={1} Kotlin={2}")
    @MethodSource("gradleAndAndroidVersions")
    @DisplayName('tasks should be created lazily')
    void shouldApplyLazily(g, a, hasKotlin) {
        def result = prepareGradleRunner(g, a, hasKotlin)
                .withArguments('tasks')
                .build()

        assert !(result.output =~ /(debug|release)SourcesJar/)
    }
